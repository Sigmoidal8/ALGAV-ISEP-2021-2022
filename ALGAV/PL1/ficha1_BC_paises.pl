...
chegoLaFacil(P1,P2):-pais(P1,_,_),pais(P2,_,_),(vizinho(P1,P2);(vizinho(P1,X),vizinho(P2,X))),P1\==P2.
...
somatorio(J,J,R):-R is J+J.
somatorio(K,J,R):-J1 is J-1,somatorio(K,J1,R1), R is R1+J1,!.

divisao(X,Y,Q,R):-Y>X,Q is 0, R is X,!.
divisao(X,Y,Q,R):-X1 is X-Y,divisao(X1,Y,Q1,R1), Q is Q1+1,R is R1.